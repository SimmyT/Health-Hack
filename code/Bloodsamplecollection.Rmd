...
```{r, echo=FALSE}
Patientbloodsample$month <-factor(month(Patientbloodsample$date_coln))
...
dspdata <-dspdata %>% group_by(nbrequest, month) %>% summarise(permonth=n())
dsptemp <-ggplot(dspdata, aes(x=month, y=permonth, fill= nbrequest)) +
